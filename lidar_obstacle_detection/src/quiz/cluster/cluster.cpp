...
void proximity(const std::vector<std::vector<float>>& points, KdTree* tree, 
	float distanceTol, int curr_index, std::vector<bool>& processed, std::vector<int>& cluster)
{
  // mark processed
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, 
  ...
  std::vector<bool> processed(points.size(), false); // each index is set to True when processed
