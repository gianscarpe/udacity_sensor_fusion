...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    ProcessPointClouds<pcl::PointXYZ>* ps = new ProcessPointClouds<pcl::PointXYZ>();

    // segmentation
    std::pair<pcl::PointCloud<pcl::PointXYZ>::Ptr, pcl::PointCloud<pcl::PointXYZ>::Ptr> segs = ps->SegmentPlane(input_cloud, 100, 0.2);
    
    // cluster
    std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> clusters_cloud = ps->Clustering(segs.first, 1.0, 3, 30);

    // RENDER
    renderPointCloud(viewer, segs.second, "plane", Color(1,1,0));
    ...
    for (pcl::PointCloud<pcl::PointXYZ>::Ptr cluster: clusters_cloud)
    {
        ...
        // render box
        Box box = ps->BoundingBox(cluster);
